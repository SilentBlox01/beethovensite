...
export const Tools: React.FC = () => {
  ...
  const startRadarScan = async () => {
      ...
      const ports = [80, 443, 8080, 21];
      logRadar(`Scanning Ports: ${ports.join(',')}`);
      for (const port of ports) {
          const start = performance.now();
          try {
              await fetch(`https://${domain}:${port}`, { mode: 'no-cors', cache: 'no-cache', signal: AbortSignal.timeout(1500) });
              ...
              if (duration < 50) {
                  ...
                  if (port === 21 || port === 8080) {
                      portScoreLocal -= 10;
                      logRadar(`WARN: Non-standard port ${port} might be filtered.`);
                  }
              }
          }
      }
  ...
  const runSpsScan = async () => {
    ...
    if (nav.connection) exposedApis.push('Network Information API');
    if (window.DeviceOrientationEvent) exposedApis.push('Device Orientation');
    if (nav.deviceMemory) exposedApis.push('Device Memory API');
    if (nav.hardwareConcurrency) exposedApis.push('Hardware Concurrency');
    if (nav.permissions) exposedApis.push('Permissions API');
    if (nav.geolocation) exposedApis.push('Geolocation API');
    if ((nav.clipboard)) exposedApis.push('Clipboard API');
    if (nav.mediaDevices) exposedApis.push('Media Devices API');
    if (exposedApis.length > 8) riskScore = 2;
    else if (exposedApis.length > 4) riskScore = 1;
    else riskScore = 0;
    const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    const brokers = ['Google Analytics', 'Meta Pixel', 'Amazon AdSystem', 'Mixpanel'];
    if (timezone.includes('America') || timezone.includes('New_York')) {
        brokers.push('Acxiom', 'Experian', 'Equifax', 'Axiom');
    } else if (timezone.includes('Europe') || timezone.includes('London') || timezone.includes('Paris')) {
        brokers.push('Criteo', 'Taboola', 'NextRoll', 'Neustar');
    } else if (timezone.includes('Asia') || timezone.includes('Tokyo')) {
        brokers.push('DMP Networks', 'Rakuten Advertising');
    }
    setSpsResult({
  ...
  const generateNoise = async () => {
    ...
    setSpsNoiseQueries([...queries]);
    for (let i = 0; i < 6; i++) {
        const q = queries[i];
        window.open(`https://www.google.com/search?q=${encodeURIComponent(q)}`, `_blank_${i}`, 'width=800,height=600');
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                {[
                                    ...
                                    <button 
                                        key={i}
                                        onClick={() => opt.s(!opt.v)}
                                        className={`py-4 px-2 rounded-2xl border-2 font-bold transition-all duration-200 ${
                                            opt.v 
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="h-4 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden flex shadow-inner">
                                        <div className={`h-full transition-all duration-1000 ease-out rounded-full ${
                                            entropy < 50 ? 'bg-rose-500 w-1/3' : entropy < 80 ? 'bg-amber-500 w-2/3' : 'bg-emerald-500 w-full'
                                        }`}></div>
                                    </div>
                                </div>
                    ...
                    <ToolCard title={t.tools.checksumTitle} icon={<ShieldCheck size={24} />} color="teal">
                        ...
                        {checksumHash && (
                            <div className="mt-8 border-t border-slate-200 dark:border-slate-800 pt-8 animate-fade-in">
                                ...
                                />
                                {checksumCompare && (
                                    <div className={`mt-6 p-6 rounded-2xl flex items-center gap-4 font-bold border-2 ${
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className={`p-8 rounded-3xl border-2 transition-colors duration-500 ${
                                    radarMetrics.status === 'safe' ? 'border-green-500/30 bg-green-500/10' : 
                                    ...
                                    <div className={`text-3xl font-black tracking-tight ${
                                        radarMetrics.status === 'safe' ? 'text-green-400' : 
                        ...
                        {spsResult && (
                            <div className="animate-fade-in space-y-8">
                                <div className={`p-6 rounded-2xl border-2 flex items-center justify-between shadow-lg ${
                                    spsResult.risk === 'High' ? 'bg-red-500/10 border-red-500 text-red-400 shadow-red-900/20' :
